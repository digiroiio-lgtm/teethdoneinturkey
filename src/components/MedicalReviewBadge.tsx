import Link from "next/link";

interface MedicalReviewBadgeProps {
  reviewerName?: string;
  reviewerTitle?: string;
  reviewerHref?: string;
  reviewedDate?: string;
}

export default function MedicalReviewBadge({
  reviewerName,
  reviewerTitle,
  reviewerHref,
  reviewedDate,
}: MedicalReviewBadgeProps) {
  const hasVerifiedReviewer = Boolean(reviewerName && reviewedDate);

  return (
    <div className="flex items-start gap-3 bg-gradient-to-br from-blue-50 to-white border border-blue-100 rounded-xl px-4 py-3 text-sm mb-6 shadow-sm shadow-blue-900/5">
      <div className="bg-gradient-to-br from-[#1e40af] to-blue-600 text-white rounded-full w-9 h-9 flex-shrink-0 flex items-center justify-center font-bold text-base mt-0.5 shadow-sm">
        🦷
      </div>
      <div>
        {hasVerifiedReviewer ? (
          <>
            <p className="font-semibold text-gray-900">
              Medically reviewed by{" "}
              {reviewerHref ? (
                <Link href={reviewerHref} className="text-[#1e40af] hover:underline">
                  {reviewerName}
                </Link>
              ) : (
                <span className="text-[#1e40af]">{reviewerName}</span>
              )}
            </p>
            {reviewerTitle && <p className="text-gray-500 text-xs">{reviewerTitle}</p>}
            <p className="text-gray-400 text-xs mt-0.5">Last reviewed: {reviewedDate}</p>
            <p className="text-gray-400 text-xs mt-0.5">
              <Link href="/editorial-policy" className="hover:underline">Review methodology</Link>
            </p>
          </>
        ) : (
          <>
            <p className="font-semibold text-gray-900">Clinical information notice</p>
            <p className="text-gray-500 text-xs">
              This page contains general dental information and does not claim a completed named medical review.
              See our{" "}
              <Link href="/editorial-policy" className="text-[#1e40af] hover:underline">
                editorial and medical review policy
              </Link>.
            </p>
          </>
        )}
      </div>
    </div>
  );
}
